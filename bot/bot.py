...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if started with promote parameter
    args = context.args
...
async def handle_payment_confirmation(query, context, package):
    ...
    # Store that we're awaiting wallet address
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle wallet address for payment verification
    if context.user_data.get('awaiting_wallet'):
        ...
        # Payment verified! Create sponsorship
        try:
...
async def check_milestones(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for alert in alerts:
            ...
            # Determine emoji based on multiplier
            emoji = "🚀"
            if multiplier >= 10:
                emoji = "🌙"
            elif multiplier >= 8:
...
def main():
    ...
    # Schedule milestone checks every 5 minutes
